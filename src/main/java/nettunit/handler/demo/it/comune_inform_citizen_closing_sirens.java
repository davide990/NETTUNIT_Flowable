...
public class comune_inform_citizen_closing_sirens extends BaseHandler implements TriggerableActivityBehavior {

    private static Logger logger = LoggerFactory.getLogger(comune_inform_citizen_closing_sirens.class);

    String evolution_predicate = "informed_citizens(comune,closing)";
    ...
    public void execute(DelegateExecution execution) {
        ...
        recipients.addOne(JixelDomainInformation.OPERATORE_SIRENE);
        musaService.addRecipient(evt, recipients.toList());
        this.getMusaRabbitMQConsumerService().save(evt, taskID);

        musaService.updateEventDescription(evt, "*emergenza superata, interrompere suono sirene, ripristino delle condizioni di normalità, l'evento può considerarsi concluso*");
        this.getMusaRabbitMQConsumerService().save(evt, taskID);
